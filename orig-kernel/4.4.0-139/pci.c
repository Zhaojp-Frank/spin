...
static bool nvme_setup_prps_e(struct nvme_dev *dev, struct request *req,
		int total_len, bool from_blk)
{
	struct nvme_iod *iod = blk_mq_rq_to_pdu(req);
...
#ifdef MYFUNC
//printk("Thread %d\n",current->pid);	dma_addr = cpu_to_le64(get_dma(helper[1], helper[0]) + offset);
		dma_addr = cpu_to_le64(get_dma(helper) + offset);
#else
		dma_addr = cpu_to_le64(*(helper) + offset);
#endif
/*OLD //dma_addr = cpu_to_le64(*((u64 *)(phys_to_virt((dma_addr & (u64)(~0xFFF))))) + offset); */
	...
	if (dma_len) {
		...
		if(from_blk && Pagef(pfn_to_page(PFN_DOWN((dma_addr))))){
			helper = ((u64 *)(phys_to_virt((dma_addr & (u64)(~0xFFF)))));
#ifdef MYFUNC
//printk("Thread %d\n",current->pid); //dma_addr = cpu_to_le64(get_dma(helper[1],helper[0]) + (dma_addr & (page_size - 1)));
			dma_addr = cpu_to_le64(get_dma(helper) + (dma_addr & (page_size - 1)));
#else
			dma_addr = cpu_to_le64(*(helper) + (dma_addr & (page_size - 1)));
#endif
/*OLD dma_addr = cpu_to_le64(*((u64 *)(phys_to_virt((dma_addr & (u64)(~0xFFF))))) + (dma_addr & (page_size - 1))); */
			sg->dma_address = dma_addr;
		}
...
#ifdef MYFUNC
//printk("Thread %d\n",current->pid); //dma_addr = cpu_to_le64(get_dma(helper[1],helper[0]));
		  dma_addr = cpu_to_le64(get_dma(helper));
#else
      dma_addr = cpu_to_le64(*(helper));
#endif
			/*OLD dma_addr = cpu_to_le64(*((u64 *)(phys_to_virt((dma_addr))))); */
			sg->dma_address = dma_addr;
		} 
...
int nvme_setup_prps(struct nvme_dev *dev, struct request *req) {
	return nvme_setup_prps_e(dev, req, false);
}
		...
		struct nvme_command *cmnd)
{
	...
	if (!nvme_setup_prps_e(dev, req, blk_rq_bytes(req), true))
		goto out_unmap;
...
struct nvme_iod *nvme_handle_physical_address(struct nvme_dev *dev, int write,
		unsigned long addr, unsigned length) //supposed called in  nvme_unmap_user_pages
{
	int err, count;
	struct scatterlist *sg;
	struct nvme_iod *iod;
...
static int handle_pfn_pages(struct scatterlist *sg, unsigned long start,
		unsigned long length) //suppose caleld in nvme_map_user_pages
{ 
	int i = 0;
